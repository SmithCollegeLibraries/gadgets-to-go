...
# Inventory auth always uses /authn/login-with-expiry and the folioAccessToken cookie.
...
Existing databases need the local-auth schema update:

```bash
mysql -u <user> -p <database> < backend/migrations/2026-06-01-local-auth.sql
```

New Docker databases receive these columns automatically from `docker/mysql/init/001-schema-seed.sql`.
