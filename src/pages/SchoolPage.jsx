...
function SchoolPage({ isPreview = false, customStyles = {}, customInventoryData = [] }) {
  ...
  const currentInstitution = getInstitutionBySlug(school);
  const getLocationName = (code) => getInstitutionByCode(code)?.name || code;
  const getSchoolName = (code) => getInstitutionByCode(code)?.name || code;
  const configuredBranches = currentInstitution?.branches || EMPTY_LIST;
  const getBranchName = (code) => configuredBranches.find((branch) => branch.code === code)?.name || code;
  ...
  }, [groupByBranch, isPreview, location.search, navigate, searchQuery, selectedBranch, showAvailableOnly, viewMode]);
  ...
  }, [fetchLayoutData, fetchInventoryData, loadConfig, school, isPreview]);
  ...
  useEffect(() => {
    if (!isPreview) {
      const fetchDisabledBranches = async () => {
        try {
          const institution = getInstitutionBySlug(school);
          const owner = institution?.code;
          ...
          // If disabled branch settings are unavailable, keep the catalog usable.
          setDisabledBranches([]);
        }
      };
  ...
  }, [school, baseUrl, isPreview, appConfig.institutions, getInstitutionBySlug]);
  ...
  const filteredBranches = useMemo(() => {
    const prefixes = currentInstitution?.branchPrefixes || [];
    return configuredBranches.filter(b =>
      (prefixes.length === 0 || prefixes.some((prefix) => b.code.startsWith(prefix))) && !disabledBranches.includes(b.code)
    );
  }, [configuredBranches, currentInstitution, disabledBranches]);
          ...
          <CardBody className="d-flex flex-column">
            <div className="mb-2">
              ...
              {(Array.isArray(item.branches) ? item.branches : (item.branch ? [item.branch] : [])).map((branchCode, idx) => (
                <Badge key={idx} color="dark" className="text-white me-1" pill aria-label={`Available at ${getBranchName(branchCode)}`}>{getBranchName(branchCode)}</Badge>
              ))}
            </div>
            <CardTitle tag="h2" className="fw-bold mb-2" style={{ color: theme.title, fontSize: '1.25rem' }}>{item.title}</CardTitle>
            <CardText tag="div" className="small mb-3 flex-grow-1 text-truncate-3" style={{ color: theme.desc }}>
                ...
                <td>
                  {(Array.isArray(item.branches) ? item.branches : (item.branch ? [item.branch] : [])).length > 0 ? (
                    <span className="text-dark small fw-medium">
                      {(Array.isArray(item.branches) ? item.branches : (item.branch ? [item.branch] : [])).map(b => getBranchName(b)).join(', ')}
                    </span>
                  ) : (
                ...
                Object.keys(groupedItems).sort().map(branchCode => (
                  <section key={branchCode} className="mb-5 fade-in" aria-labelledby={`section-${branchCode}`}>
                    {branchCode !== 'Unassigned' && (
                      <h2 id={`section-${branchCode}`} className="mb-3 border-bottom pb-2" style={{ color: theme.headerText }}>{getBranchName(branchCode)}</h2>
                    )}
                    {viewMode === 'grid' ? (
                      <Row>
